...
6. **동적 검증(webapp-testing)**: 화면 완성 후 webapp-testing 스킬을 로드해 dev 서버에서
   실제 브라우저로 검색·페이지네이션·행 확장·역할 전환을 검증하고, 핵심 상태 스크린샷을
   `docs/verification/<라우트>/`에 저장한다. 스킬 미설치 시 그 사실을 보고하고 수동 확인으로 대체한다
...
- 업로드 템플릿(.xlsx) 파일 생성에는 xlsx 스킬을 로드해 사용한다 (미설치 시 보고)
...
- [ ] 검색 패널 각 필드 + 페이지네이션이 실제 동작 (webapp-testing 브라우저 검증)
- [ ] 목 세션 OPERATOR ↔ CLIENT 전환: 데이터 스코핑·필터 노출·라우트 가드 차이 확인 (스크린샷 증빙)
- [ ] 상태 표시가 라벨 맵 경유, 확정 명칭과 일치
- [ ] lib/data 외 데이터 접근 0건

## 📤 완료 보고 형식

1. 구현한 라우트·파일 목록, 사용한 lib/data 함수·공통 컴포넌트, 검증 스크린샷 경로
